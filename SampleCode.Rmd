...
* The `<<EndOfSqlite3Commands ... EndOfSqlite3Commands` syntax is a bash feature called 'here-documents' (see [docs](https://tldp.org/LDP/abs/html/here-docs.html)) 
* The list of commands between the `EndOfSqlite3Commands` delimiters is redirected to the sqlite program. 
* This means this code chunk can be run in a script, as it doesn't trigger the sqlite interactive mode.
* If you would prefer to enter interactive mode and run these commands one by one, just skip the `EndOfSqlite3Commands` delimiters
