...
* **reference_allele**: The reference allele of the SNP
...
The process of SNP extraction is outlined in Figure 1:

```{r structure, echo=FALSE,fig.cap='Figure 1: Structure of SNP extraction',fig.align='center',fig.width=15,fig.height=5}
img1<-readPNG('K:\\TEU\\TechPRS\\Figures\\Extract_SNP.PNG')
...
* `SELECT Variant.* FROM Variant` selects all columns from the Variant table (`*` is a wildcard)
* `Variant.chromosome` selects the 'chromosome' column from the Variant table
* `UNION` is used because we don't know if the allele labels (effect vs reference) will be consistent between our Betas.csv and the BGEN data
...
Having printed the list of allele frequencies, we can use `awk` to quickly identify palindromic SNPs (ie SNPs where both alleles are (A, T) or (C, G)) which have effect allele frequencies close enough to 0.5 that we cannot unambiguously distinguish between the effect and reference alleles. 

In this case, we will consider "ambiguous" SNPs to be those with EAF between 0.49 and 0.51. 
